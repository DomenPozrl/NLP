results = {}
for filename in ['grid_search_domen.txt', 'grid_search_jan.txt', 'grid_search_jagos.txt']:
    f = open(filename, 'r')
...
for params in results:
    f1 = ((results[params]['per'][0] * results[params]['per'][1]) +
          (results[params]['org'][0] * results[params]['org'][1]) +
           ...
           (results[params]['misc'][0] * results[params]['misc'][1]) +
           (results[params]['deriv-per'][0] * results[params]['deriv-per'][1])) / (results[params]['per'][1] +
                                                                                   results[params]['org'][1] +
                                                                                   results[params]['loc'][1] +
                                                                                   results[params]['misc'][1] +
                                                                                   results[params]['deriv-per'][1]
                                                                                   )
    if f1 > max:
        max = f1
        max_params = params
